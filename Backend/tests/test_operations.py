...
def test_ais_update_missing_fields(unit_client):
    r = unit_client.post("/api/v1/ops/visits/1/ais", json={})
    assert r.status_code == 422


def test_ais_update_invalid_lat(unit_client):
    r = unit_client.post("/api/v1/ops/visits/1/ais", json={
...
def test_ais_update_invalid_lon(unit_client):
    r = unit_client.post("/api/v1/ops/visits/1/ais", json={
...
async def test_ais_update_live(client):
    u = uid()
    vr = await client.post("/api/v1/vessels/", json={
    ...
    r = await client.post(f"/api/v1/ops/visits/{visit_id}/ais", json={
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_weather_impact_live(client):
    u = uid()
    vr = await client.post("/api/v1/vessels/", json={
    ...
    r = await client.get(f"/api/v1/ops/visits/{visit_id}/weather-impact")
    assert r.status_code == 200
    data = r.json()
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_ais_update_not_found(client):
    r = await client.post("/api/v1/ops/visits/999999/ais", json={
