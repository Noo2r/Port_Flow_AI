...
async def test_metrics_live(auth_client):
    r = await auth_client.get("/api/v1/analytics/metrics")
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_metrics_counts_grow_with_data(auth_client):
    """After creating a vessel, total_vessels should increase."""
    from tests.conftest import uid

    before = (await auth_client.get("/api/v1/analytics/metrics")).json()
    vessel_count_before = before["total_vessels"]

    await auth_client.post("/api/v1/vessels/", json={
        "imo_number": f"IMO9{uid()}", "name": "Analytics Vessel",
        "vessel_type": "tanker", "flag": "GR",
    })

    after = (await auth_client.get("/api/v1/analytics/metrics")).json()
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_berth_utilization_range(auth_client):
    """Utilization must be between 0 and 100."""
    data = (await auth_client.get("/api/v1/analytics/metrics")).json()
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_completed_visits_nonzero_after_simulation(auth_client):
