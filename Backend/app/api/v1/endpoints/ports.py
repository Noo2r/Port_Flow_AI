...
async def create_port(payload: PortCreate, db: AsyncSession = Depends(get_db)):
...
async def update_port(port_id: int, payload: PortUpdate, db: AsyncSession = Depends(get_db)):
    port = await db.get(Port, port_id)
    if port is None:
        raise HTTPException(status_code=404, detail="Port not found")
