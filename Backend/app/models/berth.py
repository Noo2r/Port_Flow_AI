...
class Berth(Base):
    ...
    has_cold_ironing: Mapped[bool] = mapped_column(Boolean, default=False)
    has_fresh_water: Mapped[bool] = mapped_column(Boolean, default=True)
    has_bunker_facility: Mapped[bool] = mapped_column(Boolean, default=False)

    # Operational
    priority_level: Mapped[int] = mapped_column(Integer, default=5)  # 1=highest, 10=lowest
    quay_length: Mapped[float | None] = mapped_column(Float, nullable=True)  # meters
    ...
    # Relationships
    port: Mapped["Port"] = relationship("Port", back_populates="berths")  # noqa: F821
