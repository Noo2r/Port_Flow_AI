...
class Prediction(Base):
    ...
    model_registry_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("ai_model_registry.id"), nullable=True, index=True
    )
    ...
    # Model info (kept for backwards compatibility when no registry entry)
    ...
    model_registry: Mapped["AIModelRegistry"] = relationship(  # noqa: F821
        "AIModelRegistry", back_populates="predictions"
    )
